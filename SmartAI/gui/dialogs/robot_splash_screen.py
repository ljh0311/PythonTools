"""
Splash screen dialog for the Smart Robot System application.

This module provides the splash screen that displays while the robot system is loading,
with the same appearance and behavior as the original but customized for robot control.
"""
...
class RobotSplashScreen:
    """A modern splash screen that displays while the robot system is loading"""
    ...
    APP_TITLE = "Smart Home Robot Assistant"
    APP_TAGLINE = "Your intelligent home companion"
    INITIAL_STATUS = "Initializing robot system..."
    ...
    # Color scheme - Smart Home Theme
    COLORS = {
        "primary": "#4285f4",  # Warm blue (Google Home style)
    ...
    # Progress animation settings - customized for robot system
    PROGRESS_STEPS = {
    ...
    def _get_version_text(self):
        """Return the version text from settings."""
        settings = get_settings()
        version = settings.get('ROBOT_CONFIG', {}).get('version', '2.0')
